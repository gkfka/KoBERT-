...
def main(parser):
    ...
    for _epoch, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            log_likelihood, sequence_of_tags = model(x_input.to(device), token_type_ids.to(device), y_real.to(device))
            ...
            if (step + 1) % model_config.gradient_accumulation_steps == 0:
                ...
                with torch.no_grad():
                    ...
                    sequence_of_tags = torch.tensor(sequence_of_tags, device=device)
                    print("sequence_of_tags: ", sequence_of_tags.to(device))
                    print("y_real: ", y_real.to(device))
                    print("loss: ", loss)
                    print("(sequence_of_tags == y_real): ", (sequence_of_tags.to(device) == y_real.to(device)))

                    mb_acc = (sequence_of_tags == y_real.to(device)).float()[y_real.to(device) != vocab.PAD_ID].mean()
