...
class NamedEntityRecognitionDataset(Dataset):
    def __init__(self, train_data_dir: str, model_dir=Path('data_in')) -> None:
        ...
        list_of_total_source_no, list_of_total_source_str, list_of_total_target_str, list_of_ner = self.load_data(train_data_dir=train_data_dir)

        self.create_ner_dict(list_of_total_target_str, list_of_ner)
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        # preprocessing
        # str -> id -> cls, sep -> pad
        # input(''.join(self._corpus[idx]))
        token_ids_with_cls_sep, tokens, prefix_sum_of_token_start_index = self._transform_source_fn(''.join(self._corpus[idx]).lower())
        # print("prefix\n")
        # print(prefix_sum_of_token_start_index)

        list_of_ner_ids, list_of_ner_label = self._transform_target_fn(''.join(self._label[idx]), tokens, prefix_sum_of_token_start_index)
        ...
        # print("-"*10)
        # print("x_input : ", len(x_input), x_input.shape)
        # print(x_input)
        # print("token_type_ids : ", len(token_type_ids), token_type_ids.shape)
    ...
    def create_ner_dict(self, list_of_total_target_str, list_of_ner):
        ...
        if not os.path.exists(self.model_dir / "ner_to_index.json"):
            ...
            # regex_ner = re.compile('<(.+?):[\w]*>')
            #
            # list_of_ner_tag = []
            # for label_text in list_of_total_target_str:
            #     # print(label_text)
            #     regex_filter_res = regex_ner.finditer(str(label_text))
            #     # a = len(regex_filter_res)
            #     for match_item in regex_filter_res:
            #         # print(match_item)
            #         # aa = re.findall(':([\w]*)', str(i[0]))
            #         ner_tag = ''.join(re.findall(':([\w]*)', str(match_item[0])))
            #         # input(match_item[1])
            #         input(ner_tag)
            ...
            for ner_tag in list_of_ner:

                ner_to_index[ner_tag] = len(ner_to_index)
    ...
    #     """ if you want to build new json file, you should delete old version. """
    #     if os.path.exists(self.model_dir / "ner_to_index.json"):
    #     # if not os.path.exists(self.model_dir / "ner_to_index.json"):
    #         # regex_ner = re.compile('<(.+?):[A-Z]{3}>')
    #         regex_ner = re.compile('<(.+?):[\w]*>')
    #
    ...
    def load_data(self, train_data_dir):
        ...
        # print("num of files: ", len(file_path))

        list_of_total_source_no, list_of_total_source_str, list_of_total_target_str, list_of_ner = [], [], [], []

        list_of_source_no, list_of_source_str, list_of_target_str, list_of_ner = self.load_data_from_txt(file_full_name=file_path)
        ...
        return list_of_total_source_no, list_of_total_source_str, list_of_total_target_str, list_of_ner
        ...
        with codecs.open(file_full_name, "r", "utf-8") as io:
            ...
            for line in lines:
                ...
                if not line[:3] == "## ":
                    ...
                    if tt not in list_of_ner:
                        list_of_ner.append(tt)


                prev_line = line

        return list_of_source_no, list_of_source_str, list_of_target_str, list_of_ner
...
class NamedEntityRecognitionFormatter():
    ...
    def transform_target_fn(self, label_text, tokens, prefix_sum_of_token_start_index):
        ...
        regex_ner = re.compile('<(.+?):[\w]*>')  # NER Tag가 2자리 문자면 {3} -> {2}로 변경 (e.g. LOC -> LC) 인경우
        ...
        # tag lens is diffrent. you need to change the code
        count_of_match = 0
        for match_item in regex_filter_res:

            ner_tag = ''.join(re.findall(':([\w|\W]*)', str(match_item[0])))
            ner_tag = ner_tag[:-1]
            if ':' in ner_tag:
                ner_tag = ner_tag[ner_tag.rindex(':')+1:]
            ner_text = match_item[1]  # <4일간:DUR> -> 4일간
            start_index = match_item.start() - (3+len(ner_tag)) * count_of_match  # delete previous '<, :, 3 words tag name, >'
            end_index = match_item.end() - (3+len(ner_tag)) - (3+len(ner_tag)) * count_of_match
        ...
        for tup in zip(tokens, prefix_sum_of_token_start_index):
            ...
            if entity_index < len(list_of_tuple_ner_start_end):
                ...
                if start <= index and index < end:  # <13일:DAT>까지 -> ('▁13', 10, 'B-DAT') ('일까지', 12, 'I-DAT') 이런 경우가 포함됨, 포함 안시키려면 토큰의 length도 계산해서 제어해야함
                    ...
                    entity_ner = list_of_ner_text[entity_index]
                    if is_entity_still_B is True:
                        entity_tag = 'B_' + entity_tag
                        list_of_ner_label.append(entity_tag)
                        is_entity_still_B = False
                    elif is_entity_still_B is False and len(token):
                        entity_tag = 'I_' + entity_tag
                        list_of_ner_label.append(entity_tag)
                else:
                    is_entity_still_B = True
                    entity_tag = 'O'
